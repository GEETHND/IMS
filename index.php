<?php
   ...
   if (isset($_POST['btnLogin']) || $_POST['btnLogin']=="Login")
	{
	$tusername=$_REQUEST['username'];echo "<script> alert('".$tusername."');</script>";
	...
	$result= mysql_query($sql) or die("<script> alert('Invalid User Name or Password1'); document.history(-1);</script>");
	if($row=mysql_fetch_assoc($result))
	{
	
	if($row['login']==0)
	{	
		echo "<script> alert('Invalid User Name or Password2'); document.history(-1);</script>";
	}
	else
	{	
		$_SESSION["login"]= true;
	...
	<div id="contents" align="center">
		...
		<div class="body clearfix">
			<table>
					...
					<tr><td>&nbsp;</td><td></td><td><!--a href="inventory.html" class="btn2">Proceed</a--><input name="btnLogin" type="submit" class="btn2" id="btnLogin" value="Login" /></td></tr>
					</form>
			</table>
		</div>
	</div>
